...
# Project Description
This project implements a simple Attribute-Based-Access-Control (ABAC) authorization engine. 

### Authorization pipeline
1. The main file loads the json file as raw data utilizing the provided loader.py module
2. The main file passes the raw json to the verify_json.py module 
    ...
    3. The data is stored in a more practical datatype (eg : subjects are stored as a dict with the id as a key)
    4. An exception is raised if the file is wrongly formatted 
    5. Only top level keys are checked, the engine and test modules will safely handle nested data that is wrongly formatted
...
- The implementation utilizes dictionaries for fast lookup of subjects and resources. 
...
When get is used and the key is missing the default value has been chosen to make the test and the policy fail. 
```py
...
- Adding new top level keys to subjects, resources, requests or policies is not easily implementable. However adding new nested fields (eg. to request_context or policies rules) can be safely done as the verify_json.py module does not enforce them. 
...
### Windows
``` bash
(.venv) PS C:\Users\user_1\OneDrive\Documenti\GitHub\Cybersecurity_Class_2026\topic-a-authorization>  py main.py --input input --output output\output.json
```

### Linux / macOS 
The software has not been tested on Linux or macOS hardware
...
- `topic-a-authorization`
    ...
    - `requirements.txt`: empty as only standard libraries are used
